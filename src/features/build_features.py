...
subset["acc_y"].reset_index(drop=True).plot(ax=ax[0], label="raw_data")
...
acc_r  = (df_squared["acc_x"]**2 + df["acc_y"]**2 + df["acc_z"]**2 )
gyr_r  = (df_squared["gyr_x"]**2 + df["gyr_y"]**2 + df["gyr_z"]**2 )
...
df_temporal = df_temporal.drop(columns=["duration"])
